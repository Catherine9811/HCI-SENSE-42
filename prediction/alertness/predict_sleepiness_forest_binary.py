...
"""
Binary sleepiness prediction script based on per-participant percentile thresholds.

For each participant, a binary target is created from the original continuous
sleepiness outcome: values above a specified within-participant percentile are
labelled 1 ("yes"), and values at or below that percentile are labelled 0 ("no").

Cross-validation is then run (within-participant and cross-participant), and
the overall TP, FP, FN, TN, precision, recall and F1 are reported.
"""
...
def create_binary_target_per_participant(
    ...
    def _per_group(g: pd.DataFrame) -> pd.DataFrame:
        ...
        # Low alertness (bottom part)
        new_vals[g[outcome_col] <= lower_th] = 0
        # High sleepiness (top part)
...
def within_participant_cv_binary(
    df: pd.DataFrame,
    feature_cols: List[str],
    target_col: str,
    group_col: str,
    ...
    for _, idx in tqdm(
        ...
        desc="  Within-participant CV (binary): participants",
        leave=False,
    ):
        ...
        for train_idx, test_idx in cv.split(X_p, y_p):
            ...
            model = build_pipeline(
                numeric_features, categorical_features, task_type="classification", random_state=RANDOM_STATE
            )
...
def cross_participant_cv_binary(
    df: pd.DataFrame,
    feature_cols: List[str],
    target_col: str,
    group_col: str,
    ...
    progress = tqdm(
        ...
        desc="  Cross-participant CV (binary): participants",
        leave=False,
    )
...
def run_binary_experiments(
    ...
    """
    Run binary classification experiments for all feature groups using the given
    ...
    for group_name, raw_features in tqdm(
        ...
        print("\n" + "=" * 80)
        print(f"Feature group (binary): {group_name}")
        ...
        wp_results = within_participant_cv_binary(
            df_bin,
            existing_features,
            target_col=BINARY_TARGET_COL,
            group_col=GROUP_COL,
        )
        cp_results = cross_participant_cv_binary(
            df_bin,
            existing_features,
            target_col=BINARY_TARGET_COL,
            group_col=GROUP_COL,
        ...
        print_results("Within-participant CV (binary)", wp_results)
        print_results("Cross-participant CV (Leave-One-Participant-Out, binary)", cp_results)


def main() -> None:
    # Default to keeping bottom/top 33% and discarding middle 33% per participant.
    run_binary_experiments(lower_percentile=1.0 / 3.0, upper_percentile=2.0 / 3.0)
