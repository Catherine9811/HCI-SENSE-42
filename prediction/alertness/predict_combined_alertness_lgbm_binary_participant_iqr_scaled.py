...
"""
Binary combined-alertness prediction script using LightGBM.
...
For each participant:
  - Build tlx_binary + sleepiness_binary from lower/upper percentiles.
Numeric features are rescaled with participant-level median/IQR (stable IQR fallback),
then cross-validation is performed on the pre-scaled data.
"""
...
PR_CURVE_WITHIN_PATH = (
    BASE_DIR / "processed_data" / "combined_alertness_lgbm_binary_participant_iqr_scaled_pr_curve_within.png"
)
PR_CURVE_CROSS_PATH = (
    BASE_DIR / "processed_data" / "combined_alertness_lgbm_binary_participant_iqr_scaled_pr_curve_cross.png"
)
...
def create_alertness_binary_target_per_participant(
    ...
    df[new_col] = df[TLX_COL] + df[SLEEPINESS_COL]
...
def fit_participant_iqr_scaler(
    ...
    if not numeric_features:
        empty_df = pd.DataFrame(index=groups_ref.unique())
        empty_series = pd.Series(dtype=float)
        return empty_df, empty_df, empty_series, empty_series

    x_num = X_ref[numeric_features]
    participant_medians = x_num.groupby(groups_ref).median()
    participant_q75 = x_num.groupby(groups_ref).quantile(0.75)
    participant_q25 = x_num.groupby(groups_ref).quantile(0.25)
...
def within_participant_cv_binary_lgbm(
    df: pd.DataFrame,
    feature_cols: List[str],
    target_col: str,
    group_col: str,
    ...
    for _, idx in tqdm(
        ...
        desc="  Within-participant CV (LGBM, binary, participant-IQR scaled): participants",
        leave=False,
    ):
...
def cross_participant_cv_binary_lgbm(
    df: pd.DataFrame,
    feature_cols: List[str],
    target_col: str,
    group_col: str,
    ...
    progress = tqdm(
        ...
        desc="  Cross-participant CV (LGBM, binary, participant-IQR scaled): participants",
        leave=False,
    )
...
def run_binary_lgbm_experiments(
    ...
    print(
        f"Definition: {BINARY_TARGET_COL}=1 defined by upper percentile of combined alertness."
    )
    print(
    ...
    for group_name, raw_features in tqdm(
        ...
        desc="Feature groups (LGBM, binary, participant-IQR scaled)",
    ):
        ...
        print("\n" + "=" * 80)
        print(f"Feature group (LGBM, binary, participant-IQR scaled): {group_name}")
        ...
        wp_results, wp_y_true, wp_y_score = within_participant_cv_binary_lgbm(
            df_bin,
            existing_features,
            target_col=BINARY_TARGET_COL,
            group_col=GROUP_COL,
        )
        cp_results, cp_y_true, cp_y_score = cross_participant_cv_binary_lgbm(
            df_bin,
            existing_features,
            target_col=BINARY_TARGET_COL,
            group_col=GROUP_COL,
        ...
        print_results("Within-participant CV (LGBM, binary, participant-IQR scaled)", wp_results)
        print_results(
            "Cross-participant CV (Leave-One-Participant-Out, LGBM, participant-IQR scaled)",
            cp_results,
        )
        ...
        if wp_y_true.size > 0 and wp_y_score.size > 0:
            wp_pr_auc = save_pr_curve(
                wp_y_true,
                wp_y_score,
                "Within-participant PR Curve (LGBM, binary, participant-IQR scaled)",
                PR_CURVE_WITHIN_PATH,
            )
        ...
        if cp_y_true.size > 0 and cp_y_score.size > 0:
            cp_pr_auc = save_pr_curve(
                cp_y_true,
                cp_y_score,
                "Cross-participant PR Curve (LGBM, binary, participant-IQR scaled)",
                PR_CURVE_CROSS_PATH,
            )
...
def main() -> None:
    ...
    run_binary_lgbm_experiments(lower_percentile=0.33, upper_percentile=0.66)
