...
SHAP_IMPORTANCE_PATH = (
    BASE_DIR / "processed_data" / "proxy_error_lgbm_shap_importance.csv"
)
SELECTED_FEATURES_PATH = (
    BASE_DIR / "processed_data" / "proxy_error_lgbm_selected_features.csv"
)
...
def run_shap_feature_selection_experiment(
    ...
    existing_features = [f for f in raw_features_full if f not in proxy_set]
    if not existing_features:
        print("No features found for PROXY_FEATURE_SET.")
        return

    print("\n" + "=" * 80)
    print("Proxy error target — SHAP feature selection on set (all safe features: behavioural + HCI excl. proxy cols)")
    ...
    baseline_wp, baseline_wp_y_true, baseline_wp_y_score = within_participant_cv_binary_lgbm(
        df_proxy, existing_features, target_col=PROXY_TARGET_COL, group_col=GROUP_COL
    )
    ...
    reduced_wp, reduced_wp_y_true, reduced_wp_y_score = within_participant_cv_binary_lgbm(
        df_proxy, selected_features, target_col=PROXY_TARGET_COL, group_col=GROUP_COL
    )
    ...
    print(f"  PR-AUC (within): {baseline_wp_pr_auc:.3f}  baseline: {_compute_pr_baseline(baseline_wp_y_true):.3f}")
    print(f"  PR-AUC (cross):  {baseline_cp_pr_auc:.3f}  baseline: {_compute_pr_baseline(baseline_cp_y_true):.3f}")
    ...
    print(f"  PR-AUC (within): {reduced_wp_pr_auc:.3f}  baseline: {_compute_pr_baseline(reduced_wp_y_true):.3f}")
    print(f"  PR-AUC (cross):  {reduced_cp_pr_auc:.3f}  baseline: {_compute_pr_baseline(reduced_cp_y_true):.3f}")
