...
class ComputerUsageHoursExtractor:
    name = "usage_hours"

    def __init__(self, csv_path):
        self.enrollment = pd.read_csv(csv_path)
        self.id_column = "Participant ID"
        self.uh_column = "Computer Usage: On average, how many hours per day do you spend using a computer?"
...
if __name__ == '__main__':
    ...
    condition_definitions = ComputerUsageHoursExtractor(enrollment)
    ...
    for psydat_file in tqdm(psydat_files):
        ...
        for extractor in variable_definitions:
            ...
            if "hours" not in processed:
                processed["hours"] = []
            processed["hours"].extend([condition_definitions.process(participant_id)] * len(predictor_values))
