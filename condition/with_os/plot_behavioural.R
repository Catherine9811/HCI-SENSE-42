...
# Load ANOVA results
results <- read_csv("processed_data/behavioural/42-operating_system-output.csv")
...
for (i in seq_len(nrow(sig_results))) {
  ...
  plot_data <- data.frame(
    Group = factor(c("Preferred OS", "Non-Preferred OS"), levels = c("Preferred OS", "Non-Preferred OS")),
    Mean = c(row$P_mean, row$N_mean),
    SE = c(row$P_se, row$N_se)
  )
  ...
  p <- ggplot(plot_data, aes(x = Mean, y = Group, fill = Group)) +
    ...
    scale_fill_manual(values = c("Preferred OS" = "#4CAF50", "Non-Preferred OS" = "#E53935")) +
    labs(
      title = row$Label,
