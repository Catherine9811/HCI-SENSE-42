...
outcome_variable <- "time"

# Read the data from the CSV file
data <- read.csv(paste("processed_data/behavioural/42-sleepiness.csv", sep=""), sep=",")
...
for (predictor in predictors) {
  ...
  formula <- as.formula(paste(predictor, "~", outcome_variable, "+ (1 | participant)"))
  ...
  estimate <- model_summary$coefficients[outcome_variable, "Estimate"]
  conf_int <- confint(model, level = 0.95)[outcome_variable, ]
  p_value <- model_summary$coefficients[outcome_variable, "Pr(>|t|)"]
