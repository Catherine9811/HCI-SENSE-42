...
# Performance is in inversed scale
...
# ---- MARGINAL R² PER CATEGORY (models with each group alone) ----
marginal_rows <- lapply(category_names, function(cat) {
  cat_vars <- category_list[[cat]]
  r2_cat   <- compute_r2(cat_vars)
  data.frame(
    term     = cat,
    estimate = r2_cat,
    stringsAsFactors = FALSE
...
marginal_r2 <- do.call(rbind, marginal_rows)

write.csv(
  marginal_r2,
  paste0("processed_data/42-", outcome_variable, "-multimodal-partR2-marginal.csv"),
  row.names = FALSE
)

cat("Category-level marginal R² table saved.\n")
...
colors <- c(
  ...
  "Mouse & Keyboard Interaction"           = "#E69F00",  # warm orange
...
rename_vars <- c(
  ...
  "interaction_hci" = "Mouse & Keyboard Interaction",
...
# ---- BAR PLOT: MARGINAL R² PER CATEGORY (same colour coding as pie) ----
marginal_r2_plot_df <- marginal_r2 %>% 
  mutate(
...
plot_marginal <- ggplot(marginal_r2_plot_df,
                        aes(x = term, y = estimate, fill = term)) +
  geom_col() +
  scale_fill_manual(values = colors, guide = "none") +
  labs(
    title = glue("Marginal R² for {toupper(outcome_variable)}"),
    x = "Predictor group",
    y = "Marginal R²"
  ) +
  theme_minimal() +
